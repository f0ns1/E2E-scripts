...
class RequestServer():
    ...
    def get_keys(self):
        return json2html.convert(self.rocket.e2e_fetchMyKeys().json())
...
def dict_to_file(filename, data):
    f= open(filename, "w+")
    for val in data:
        f.write(val+"\n")
    f.close()
    print("\t\t messages to file ==> ", filename)

def dump_data(request):
    to_file("./dump_dir/user_ownkeys.html",request.get_keys())
...
def main():
    ...
    if args.extract:
        print('Extract Mode ', args.extract)
        for value in args.extract.split(" "):
            print('Extract ==> ', value)
            if value == "subscriptions":
                data = request.subscriptions()
                for item in data['update']:
                    name, rid, e2ekey = None, None, None
                    try:
                        name = item['name']
                        rid = item['rid']
                        e2ekey = item['E2EKey']
                        ...
                        pass
                    print('name: ', name, ' rid: ', rid, ' E2EKey: ', e2ekey)
                ...
                for item in data:
                    for msg in item['messages']:
                        print('rid: ', msg['rid'], ' msg: ', msg['msg'])
                        output.append(msg['msg'])
        export = input('Import msg to file: y/n? ')
        if export == "y":
            path = input("file_path: ")
            dict_to_file(path, output)
